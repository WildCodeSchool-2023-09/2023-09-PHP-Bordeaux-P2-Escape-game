...
private function processEnigmaAnswer(string $scene, string $plan, array $planData, UserManager $userManager, ProgressManager $progressManager): array
{
    $result = [];
    ...
    if (isset($planData['enigma']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_POST[$answer])) {
            ...
            $this->processIncorrectAnswer($userScore, $scene, $userManager, $progressManager); // Appeler la méthode correcte ici
        }
    }
    ...
    private function processIncorrectAnswer(?int $userScore, string $scene, UserManager $userManager, ProgressManager $progressManager): void
{
    if ($userScore !== null) {
        $userScore -= 5;
        $userManager->updateUserScore($_SESSION['user_id'], $userScore);
...
private function processCorrectAnswer(string $scene, string $plan, ?int &$userScore, UserManager $userManager, ProgressManager $progressManager): void
{
